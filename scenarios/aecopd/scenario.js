...
function renderCenter(){center.innerHTML='';const b=document.createElement('div');b.className='patient-box';b.appendChild(createVideo(mediaForState[state]||M.act1InitialPatient));center.appendChild(b);}
...
function renderAct1(){left.innerHTML=patientBackground()+act1Controls();if(state==='ACT1_TREATMENT')renderAct1Treatment();else right.innerHTML=evidenceCards();if(state==='ACT1_LUNG_SOUND'){right.innerHTML+='<button id="play-act1-lung" class="btn dark" style="margin-top:14px">▶ Play respiratory sound</button>';document.getElementById('play-act1-lung').onclick=()=>new Audio(M.lungSound).play().catch(()=>{});}bindStates();}
function renderAct1Treatment(){right.innerHTML='<div class="eyebrow">Medical Order Wall</div><h2>Medical Order Wall</h2><div class="order-wall"><div class="order">'+esc(D.act1.treatment.airway)+'</div>'+[...act1Orders].map(x=>'<div class="order">'+esc(x)+'</div>').join('')+'</div><h3>Medication Orders</h3><div class="option-grid">'+D.act1.treatment.medicationOptions.map((o,i)=>'<button class="btn dark" data-med="'+i+'">'+esc(o.label)+'</button>').join('')+'</div><h3>Oxygen Therapy</h3><div class="option-grid">'+D.act1.treatment.oxygenOptions.map((o,i)=>'<button class="btn dark" data-o2="'+i+'">'+esc(o.label)+'</button>').join('')+'</div><div id="act1-feedback" class="feedback">'+esc(D.act1.treatment.instruction)+'</div><button id="act1-complete" class="btn dark disabled" style="margin-top:14px">30-min reassessment → Act 2</button>';
